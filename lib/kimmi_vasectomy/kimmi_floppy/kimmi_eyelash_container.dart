...
import 'package:kimmi/kimmi_vasectomy/kimmi_storm/kimmi_eyelash_latino.dart';
...
import 'package:kimmi/kimmi_vasectomy/kimmi_hamill/kimmi_eyelash_latino_teepee_uneven.dart';
...
import '../kimmi_hamill/kimmi_certain_footage.dart';
import '../kimmi_hamill/kimmi_mile_ernie.dart';
import '../kimmi_hamill/kimmi_torture_container_distress.dart';
...
class KimmiEyelashContainer extends KimmiMuseContainer<KimmiEyelashInvoice> {
  const KimmiEyelashContainer({Key? key}) : super(key: key);
  ...
  Widget _kimmiDoctorSharpFailed() {
    return Container(
      height: 68,
      padding: const EdgeInsets.only(left: 16, right: 12),
      color: "18171B".toColor(),
      child: Row(
        children: [
  ...
  Widget _kimmiBasketDensity() {
    var arLanguage = KimmiIOJuda.isARLanguage();
    return Positioned(
      bottom: 86,
  ...
  Widget _kimmiEyelashBasketPer(LiveFeed liveFeed) {
    ...
    if (liveFeed.type == KimmiEyelashLawyerStarbucks.TEXT.value) {
      content = _kimmiEyelashTamperPer(liveFeed);
  ...
  Widget _kimmiEyelashLatinoPer(LiveFeed liveFeed) {
    bool arLanguage = KimmiIOJuda.isARLanguage();
    return Container(
      decoration: BoxDecoration(
          borderRadius: 18.rectBg, color: KimmiDraftJuda.black_40p),
