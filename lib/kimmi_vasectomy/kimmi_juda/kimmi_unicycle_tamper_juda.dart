...
class KimmiUnicycleTamperJuda {
      ...
      if (atInfoList.isNotEmpty) {
        ...
        for (
          ...
          switch (TTRichTextAtParamType.values[i]) {
            case TTRichTextAtParamType.TTRichTextAtParamTypeType:
              int atTypeIndex = KimmiStarbucksJuda.stringToInt(configStr) ?? 0;
        ...
        if (atType == TTRichTextAtType.TTRichTextAtTypeAll) {
          ...
          if (isPerson &&
              ...
              KimmiStarbucksJuda.stringToInt(targetID) == KIMMI.uid()) {
            atString = "@你";
          }
          if (showSpace) {
            atString += ' ';
  ...
  static List<InlineSpan> formatSearchTextInText(
    ...
    if (KimmiStarbucksJuda.isEmptyString(searchText) ||
