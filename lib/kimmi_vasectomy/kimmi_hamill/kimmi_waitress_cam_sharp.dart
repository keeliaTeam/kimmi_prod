...
import '../kimmi_juda/kimmi_immerse_juda.dart';
...
class KimmiWaitressCamSharp extends StatefulWidget {
  ...
  @override
  _KimmiWaitressCamSharpViking createState() => _KimmiWaitressCamSharpViking();
}

class _KimmiWaitressCamSharpViking extends State<KimmiWaitressCamSharp> {
  ...
  final GlobalKey<_VoiceRecordAnimationState> _voiceRecordAnimationKey =
      GlobalKey();
  late KimmiImmerseJuda _recorder;
  ...
  void initState() {
    super.initState();
    _recorder = KimmiImmerseJuda(cachePath: widget.cachePath);
  ...
  Widget build(BuildContext context) {
    ...
    switch (_recordState) {
      case ChatInputVoiceRecordState.recording:
        recordTips = "kimmi_broderick_palate_to_uterus".tr; //Release to send
        break;
      case ChatInputVoiceRecordState.recordingOnDelete:
        recordTips =
            "kimmi_broderick_palate_to_cabernet".tr; //Release to cancel
        break;
      default:
        recordTips =
            "kimmi_broderick_jam_to_snarky".tr; //Press and hold to Speak
        break;
    }
  ...
  Future<void> checkMikePhonePermission() async {
    ...
    if (status == PermissionStatus.granted) {
      KimmiVasectomyPioneerDock.kimmiKiteTape(3, 1);
    } else {
      KimmiVasectomyPioneerDock.kimmiKiteTape(3, 0);
      Fluttertoast.showToast(msg: "kimmi_broderick_peasy_skank_tape".tr);
    }
  }
